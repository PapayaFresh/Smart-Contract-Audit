...
contract SafeBabyToken is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 private _tTotal = 14000 * 10 ** 9 * 10 ** 8;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "SafeBaby";
    ...
    function setDeadFeePercent(uint256 deadFee) external onlyOwner() {
        require(deadFee <= 30, 'Dead fee should be less than or equal to 30');
        uint256 oldDeadFee = _deadFee;
        _deadFee = deadFee;
        emit SetDeadFeePercent(oldDeadFee, deadFee);
    }
